  ...
  renderChat(step) {
    ...
    `;
      ...
      `;
          ...
          // Handle script loading before attaching click events
          const scriptsToLoad = [];

          chatData.options.forEach((option, idx) => {
              if (option.load) {
                  scriptsToLoad.push(self.loadScript(option.load));
              }
          });

          // Wait for all scripts to load, then attach click handlers
          Promise.all(scriptsToLoad).then(() => {
              ...
              // Still attach click handlers even if script loading fails
              botMsg.querySelectorAll(".option-btn").forEach((button, idx) => {
                  const option = chatData.options[idx];
